 ...
 *		sk_attno = index column number for leading column of row
 *comparison sk_strategy = btree strategy code for semantics of row comparison
 *				(ie, < <= > or >=)
 ...
 *				element (needed since row header does not include
 *a count) sk_func points to the btree comparison support function for the
 *				opclass, NOT the operator's implementation
 *function. sk_strategy must be the same in all elements of the subsidiary
 *array, that is, the same as in the header entry. SK_SEARCHARRAY,
 ...
 */
extern void ScanKeyInit(ScanKey entry, AttrNumber attributeNumber,
                        StrategyNumber strategy, RegProcedure procedure,
                        Datum argument);
extern void ScanKeyEntryInitialize(ScanKey entry, int flags,
                                   AttrNumber attributeNumber,
                                   StrategyNumber strategy, Oid subtype,
                                   Oid collation, RegProcedure procedure,
                                   Datum argument);
extern void ScanKeyEntryInitializeWithInfo(ScanKey entry, int flags,
                                           AttrNumber attributeNumber,
                                           StrategyNumber strategy, Oid subtype,
