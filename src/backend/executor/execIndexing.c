...
static bool check_fun_dep_constraint(
    struct fd_struct* fd_info,Relation heap, Relation index, IndexInfo *indexInfo, ItemPointer tupleid,
    Datum *values, bool *isnull, EState *estate, bool newIndex,
    CEOUC_WAIT_MODE waitMode, bool violationOK, ItemPointer conflictTid) ;
					  ...
					  bool onlySummarizing)
{
	...
	for (i = 0; i < numIndices; i++)
	{
		...
		if(check_fun_const){
			...
			struct fd_list* curr_itm=fd_info_list;
			char name[50];
			strncpy(name,heapRelation->rd_rel->relname.data,50);
				...
				if(test_index){
					...
					if(val){
						ereport(ERROR,
								...
								errmsg("conflicting functional dep \"%s\"",
										RelationGetRelationName(indexRelation)),
											errdetail("functipnal dep not satisfied"),
								errtableconstraint(heapRelation, RelationGetRelationName(indexRelation))));
					}
				}
		...
		if (indexInfo->ii_ExclusionOps != NULL)
		{
			...
			satisfiesConstraint =check_exclusion_or_unique_constraint(heapRelation,
													 indexRelation, indexInfo,
													 tupleid, values, isnull,
													 estate, false,
													 waitMode, violationOK, NULL);
...
static bool check_fun_dep_constraint(
	...
	//bool to_check=bool_to_check_fun_dep();
    
	
    int num_key_attr_right=fd_info->no_l_cols;
	scankeys = (ScanKey) palloc(num_key_attr_right * sizeof(ScanKeyData));
	int* key_list_right=fd_info->l_cols;
	

	int num_key_attr_left=fd_info->no_r_cols;
	int* key_list_left=fd_info->r_cols;

    for (i= 0; i < num_key_attr_right; i++) {
		int idx=find_att_num(indexInfo->ii_IndexAttrNumbers,indexInfo->ii_NumIndexAttrs,key_list_right[i]);

        ScanKeyEntryInitialize(&scankeys[i],
                               0 , idx + 1,
                               3, 25,
                               index_collations[idx], 67, values[idx]);
    }
...
retry:
    conflict = false;
    found_self = false;
    index_scan = index_beginscan(heap, index, &DirtySnapshot, num_key_attr_right, 0);
    index_rescan(index_scan, scankeys, num_key_attr_right, NULL, 0);

    while (
        index_getnext_slot(index_scan, ForwardScanDirection, existing_slot)) {
        TransactionId xwait;
        ...
        if (index_scan->xs_recheck) {
            if (!index_recheck_constraint(index, constr_procs, existing_values,
                                          existing_isnull, values))
                continue; /* tuple doesn't actually match, so no
                           * conflict */
        }
		bool cons=true;
		TupleDesc rd_att=RelationGetDescr(index);
		Form_pg_attribute att;
		bool key_equal=true;
		for(i=0;i<num_key_attr_right;i++){
            att=TupleDescAttr(rd_att, key_list_right[i] - 1);
			if(datumIsEqual(existing_values[key_list_right[i]-1],values[key_list_right[i]-1],att->attbyval,att->attlen)){key_equal=true;}
			else{key_equal=false;break;}
		}
		if(!key_equal){
			continue;
		}
		for(i=0;i<num_key_attr_left;i++){
			//have to add size constraints::

			att=TupleDescAttr(rd_att, key_list_left[i] - 1);
			if(datumIsEqual(existing_values[key_list_left[i]-1],values[key_list_left[i]-1],att->attbyval,att->attlen)){cons=cons&&true;}
			else{cons=false;break;}
		}
		conflict=!cons;
		if(!conflict){
			continue;
         ...
         */
        xwait = TransactionIdIsValid(DirtySnapshot.xmin) ? DirtySnapshot.xmin
                                                         : DirtySnapshot.xmax;
									 ...
									 ItemPointer conflictTid)
{
	...
	//*****NEED TO REMOVE WHILE ORIGINAL TESTING::
	//init_fd_list_hard_code();
