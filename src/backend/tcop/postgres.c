			...
			case 'Q':			/* simple query */
				{
					...
					if(strncmp(query_string,"INSERT FUN_DEP",14)==0){
						...
						while(ptr!=NULL){
							...
							else if(i>2 && !appear_to){
								count_left++;
								left[count_left-1]=atoi(ptr);
							}
							else if(i>2 && appear_to && strncmp(ptr,";",1)!=0){
								count_right++;
								right[count_right-1]=atoi(ptr);
							}
						...
						strcat(catquery,"INSERT INTO fd_table values('");
						strcat(catquery,table_name);
						strcat(catquery,"'");
						char l[3];
						l[0]=',';
						l[1]=count_left+'0';
						...
						strcat(catquery,"SELECT * FROM fd_table WHERE table_name='");
						strcat(catquery,table_name);
						strcat(catquery,"';");
					...
					if (am_walsender)
					{
						if (!exec_replication_command((const char*)new_buf))
							exec_simple_query((const char*)new_buf);
					}
					else
						exec_simple_query((const char*)new_buf);

					valgrind_report_error_query((const char*)new_buf);
